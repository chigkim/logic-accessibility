...
If it is bar 1 beat 1 division 1 tic 1, it should just read "1 1 1 1" instead of "000 1 1 1 1 ".
...
Logic wants to announce MBT as playhead moves, and VO wants to announce the value of the slider. Two announcements are fighting each other.

### Multisampler 
In EXS24, VO users were able to edit zones, group, and many properties in edit window. However, it seems like we cannot do that in multisampler.

It is being investigated by VO users, but at least it's not easily found how to do it.

If there is no way to do that now, it would be great to be able to do it similar to EXS24 in future updates. 
...
It would be nice to be able to trigger play by simply pressing vo+space on a cell without interacting with the cell.

 Playing live loop requires time sensitive interaction, so minimizing number of keyboard interaction is crucial.
...
It would be nice to be able to tell if a cell is playing or not without interacting with it.
...
* Input: "Input 1" as value, "input" as label, and "popup button" as role. Then it should say "Input 1 input popup button". 
* Insert: "Channel eq" as value, "Plugin" as label, "group" as role. Then it should say "Channel EQ Plugin Group" 
* Send: "Bus 1" as value, "Send" as label, "group" as role. Then it should say "bus 1 send group". 
